...
## Weekly Exercise Routine Template

Sample 12 (the exercise-routine chatbot) renders its board from the template
below instead of hard-coding it in Python. The program reads the block delimited
by the `ROUTINE-TEMPLATE` markers, substitutes `{{COLUMNS}}` (the pipe-separated
day names) and `{{ROWS}}` (one table row per day), and writes the result.

<!-- ROUTINE-TEMPLATE:BEGIN -->
```fancy-kanban
---
title: Weekly Exercise Routine
fields:
  - name: title, type: Text, label: Activity
  - name: status, type: Select, options: {{COLUMNS}}, label: Day
